        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered table-striped {{ count($properties) > 0 ? 'datatable' : '' }} @can('property_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
                        @can('property_delete')
                            @if ( request('show_deleted') != 1 )<th style="text-align:center;" class="action-delete"><input type="checkbox" id="select-all" /></th>@endif
                        @endcan
                    ...
                    @if (count($properties) > 0)
                        @foreach ($properties as $property)
                            <tr data-entry-id="{{ $property->id }}">
                                ...
                                <td field-key='status'>
                                    @if ($property->rented_date)
                                        <div style="color: rgb(45, 229, 17)">Already have Tenants</div> 
                                    @else
                                        <div style="color: red">No Tenant yet</div> 
                                    @endif
                                </td>
                                @if( request('show_deleted') == 1 )
                                <td>
